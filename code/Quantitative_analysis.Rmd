...
```{r}
# Authenticate once
gs4_auth(email = Sys.getenv("daseh_gmail")) # Replace with a valid email if needed!
...
postcourse <- get_clean_survey("2 - Post-course", "2_post-course")
...
precourse_num <- precourse |> filter(Session %in% c(1,2)) |> select(random_id, 5:19)
postcourse_num <- postcourse |> filter(Session %in% c(1,2)) |> select(random_id, 3:18)
postcodeathon_num <- postcodeathon |> filter(Session %in% c(1,2)) |> select(random_id, 3:19)
...
pdf("figures/likert_comfort_confidence.pdf", width = 7.5, height = 4)
cowplot::plot_grid(
...
summative_stats <-
  ...
  mutate(adj.p = p.adjust(`Pr(>|t|)`, method = "bonferroni"))
```
