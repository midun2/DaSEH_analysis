...
tool_labels <- list(comfort = "\nPerceived\ncomfort", confidence = "\nPerceived\nconfidence")

tools_all <- names(the_labels)

make_likert_plot <- function(tool, dat, make_hist = F) {
  cols <- paste0(tool, c("1", "2", "3"))
  ...
  if (!make_hist) {
    ...
    likert.histogram.plot(likert_obj, text.size = 3,order = rev(names(likert_tool))) +
      theme(
        axis.ticks = element_blank(),
...
likert_plots <- lapply(comf_conf_all, make_likert_plot, dat = daseh_dat_clean)
hist_plots <- lapply(comf_conf_all,
                     make_likert_plot,
                     dat = daseh_dat_clean,
                     make_hist = T)
